...
온라인 그루밍은 **성인이 인터넷을 매개로 미성년자에게 접근해 신뢰를 구축한 뒤, 점진적으로 성적 착취로 이어가는 계획적 범죄 행위**를 말한다. 단순 불건전 대화가 아니라, **장기간에 걸친 전략적 관계 조작**이라는 점에서 별도의 개념으로 취급된다.

### 핵심 특징

온라인 그루밍은 **비대칭적 권력 관계** 속에서 **신뢰 형성 → 고립 및 통제 → 성적 대화 유도 → 협박**이라는 명확한 단계를 거친다. 가해자는 친구, 연인, 멘토, 멘토 등으로 위장하며 피해자의 심리적 취약점(외로움, 자존감 부족, 가정 문제)을 악용한다. 선물, 게임 아이템, 취업 기회 같은 미끼와 함께 "우리만의 비밀", "넌 남달라"는 표현으로 **관계의 폐쇄성과 특별함**을 강조한다.

### 발생 환경과 탐지의 어려움

SNS, 메신저, 게임 채팅, 익명 커뮤니티 등 디지털 플랫폼의 비공개 대화, 익명성, 쉬운 계정 삭제 기능은 가해자가 **실제 신원을 숨긴 채 흔적 없이 활동하도록** 돕는다. 그루밍이 '범죄'로 인지되는 시점과 실제 피해 진행 단계 사이에는 **큰 시간 격차**가 존재한다. 초기 신호(신뢰 구축, 고립 시도, 비밀 강요)는 정상 대화와 구분이 어려워 많은 사례가 **3-4단계(성적 요구, 협박)까지 진행된 후에야 발견**된다.

### 피해와 인식의 간극

피해자는 자신이 조종당하고 있다는 사실을 뒤늦게 자각하며, **자책감, 수치심, 우울, PTSD** 등 심각한 정신적 후유증을 입는다. 디지털 기록의 특성상 유포된 이미지·영상은 완전한 삭제가 불가능하다. 그럼에도 현실에서는 "연인 사이 문제", "아이도 동의했다"는 식으로 축소·왜곡되기 쉽다.

### 국제 동향과 패러다임 전환의 필요성

유럽연합, 영국, 미국 등은 온라인 그루밍을 **독립된 범죄 유형**으로 규정하고 **온라인 접촉 자체만으로도 처벌**하는 입법을 확산 중이다. 우리나라도 n번방 사건을 계기로 그루밍 개념을 사회·법·정책 논의에 본격 도입하고 있다. 핵심은 **"사후 처벌"이 아닌 "초기 단계 탐지·차단"** 으로의 패러다임 전환이다.

본 프로젝트에서 온라인 그루밍 탐지 모델을 개발하는 이유는, **돌이킬 수 없는 피해가 발생하기 이전, 아직 '대화' 단계에서 개입하고 보호할 수 있는 기술적 수단을 마련하기 위해서**다.
...
#### 그루밍 대화 2
  ...
  입력 텍스트: **"You seem so much more mature than kids your age. I feel like I can                                                                                               
     ...
     <a align="center">                                                                                                                                               
     <img src="photo/bad2.png" width="500"></a>                                                                                                                       
  **분석 결과** :  
